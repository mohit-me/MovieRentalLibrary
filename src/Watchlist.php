<?php 
	...
	if (isset($_SESSION['id']))
	{
		...
		$query = "SELECT * FROM user_watchlist WHERE username ='".$_SESSION['username']."'";
		//echo $query;
		...
		while($watchitem = mysqli_fetch_array($result))
		{
			$query = 'SELECT distinct * FROM movies  WHERE mid = '.$watchitem['MID'] ;
			//echo $query;
			...
			while ($row = mysqli_fetch_array($resultproduct))
			{
				...
				echo '<div class="itemtrash"><p><a href="./includes/removeWatchlist.php?mid='.$watchitem['MID'].'">Remove</a></p></div>
					</article>';
				$colcount++;
				$totcount++;
	...
	{
		//No cart items
		echo '<article><p>There are no items in your watchlist!</p></article>';
